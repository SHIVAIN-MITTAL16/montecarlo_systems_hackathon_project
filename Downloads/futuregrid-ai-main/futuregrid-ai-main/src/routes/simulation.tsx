...
const DEFAULTS: Record<string, number> = {
  heat: 2, surge: 8, wind: 10, solar: 8, batt: 92, gas: 1.5, tx: 0,
};

const RANGES: Record<string, [number, number, number]> = {
  heat: [0, 12, 1], surge: [0, 60, 1], wind: [0, 100, 5], solar: [0, 100, 5],
  batt: [0, 100, 5], gas: [0, 12, 0.5], tx: [0, 8, 1],
};
...
export const Route = createFileRoute("/simulation")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Simulate national-scale energy crises in real time. Move the sliders — the grid responds instantly." },
...
function Simulation() {
  ...
  const sim = useMemo(() => {
    ...
    const stress = Math.min(100,
      k.heat * 4 + k.surge * 0.7 + k.wind * 0.25 + k.solar * 0.3 +
      (100 - k.batt) * 0.3 + k.gas * 5 + k.tx * 6
    );
    const states = STATES.map((s) => {
      ...
      const blackout = Math.min(95, Math.round(risk * 0.35));
      return { ...s, risk, blackout };
    });
    const affected = states.filter((s) => s.risk >= 70).length;
    const deficit = Math.round((k.surge * 0.6 + k.heat * 0.8 + k.solar * 0.05 + k.wind * 0.04 + k.gas * 0.9) * 0.6);
    const reliability = Math.max(60, +(100 - stress * 0.32).toFixed(2));
    const carbon = +((k.gas * 220 + k.surge * 18) / 10).toFixed(1);
    const cost = +((k.surge * 8 + k.heat * 12 + k.gas * 60) / 10).toFixed(1);
    ...
    return {
      stress, states, affected, deficit, reliability,
  ...
  const reasoning = useMemo<ReasoningStep[]>(() => {
    ...
    if (k.heat > 1 || k.surge > 5) {
      steps.push({
        signal: k.heat > 4 ? "Heatwave drives AC load above forecast" : "National demand rising above baseline",
    ...
    if (k.solar > 5 || k.wind > 5) {
      steps.push({
        signal: k.solar > k.wind ? "Solar generation degrading (cloud / dust)" : "Wind ramp falling below schedule",
    ...
    if (k.gas > 0.5 || k.tx > 0) {
      steps.push({
        signal: k.tx > 0 ? `Transmission corridors tripping (${k.tx})` : "Forced thermal outage detected",
    ...
    steps.push(
      sim.blackout > 25
        ? { signal: "Residual blackout risk · escalate to operator", detail: `P ${sim.blackout}%`, trend: "up" }
        : sim.blackout > 8
          ? { signal: "Risk contained · monitor next 6 h", detail: `P ${sim.blackout}%`, trend: "stable" }
          : { signal: "Grid stabilized within envelope", detail: `P ${sim.blackout}%`, trend: "resolved" }
    );
    return steps;
  ...
  const heroTone =
    sim.blackout > 25 ? { c: "oklch(0.68 0.24 25)", label: "CRITICAL" } :
    sim.blackout > 8  ? { c: "oklch(0.82 0.17 75)", label: "WATCH"   } :
    ...
    <div className="px-6 py-6 space-y-6">
      <section className="panel p-6 relative overflow-hidden">
        ...
        <h1 className="text-3xl font-display font-semibold">
          Stress-test the <span className="text-[oklch(0.72_0.18_245)] text-glow-primary">National Grid</span> · in real time
        </h1>
        <p className="text-muted-foreground mt-2 max-w-3xl">
          Move any control. The digital twin re-simulates 10,000 dispatch scenarios per second and updates state-level risk, blackout probability, and the AI recommendation instantly.
        </p>
      </section>
              ...
              <div key={knob.id}>
                <div className="flex items-baseline justify-between mb-1.5">
                  ...
                  <div className="font-mono text-sm text-[oklch(0.72_0.18_245)] tabular-nums">
                    {val}<span className="text-[10px] text-muted-foreground ml-1">{knob.unit}</span>
                  </div>
                </div>
        ...
        <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex items-baseline gap-4">
                  <div
                    ...
                    style={{ fontSize: "clamp(56px, 8vw, 104px)", color: heroTone.c, textShadow: `0 0 36px ${heroTone.c}55` }}
                  >
                    {sim.blackout}<span className="text-3xl md:text-4xl opacity-70">%</span>
                  </div>
                ...
                <div className="mt-6 flex flex-wrap gap-x-8 gap-y-3 font-mono text-[11px] text-muted-foreground">
                  <span>RESERVE · <span className="text-foreground tabular-nums">{sim.reserveMargin}%</span></span>
                  <span>DEFICIT · <span className="text-foreground tabular-nums">{sim.deficit} GW</span></span>
                  <span>AFFECTED · <span className="text-foreground tabular-nums">{sim.affected} states</span></span>
                  <span>RELIABILITY · <span className="text-foreground tabular-nums">{sim.reliability}%</span></span>
                </div>
              </div>
          ...
          <div className="panel p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="font-mono text-[10px] text-[oklch(0.85_0.21_145)]">FLOWS · {sim.states.filter(s => s.risk >= 45).length} CORRIDORS UNDER STRESS</span>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <MetricCard label="Carbon Impact" value={sim.carbon} suffix=" ktCO₂" decimals={1} tone="warning" />
            <MetricCard label="Cost Impact" value={sim.cost} prefix="₹" suffix=" Cr/hr" decimals={1} tone="primary" />
            <MetricCard label="BESS Dispatch" value={sim.battDispatchGW} suffix=" GW" decimals={1} tone="primary" />
            <MetricCard label="Composite Stress" value={Math.round(sim.stress)} suffix="" tone="destructive" />
          </div>
